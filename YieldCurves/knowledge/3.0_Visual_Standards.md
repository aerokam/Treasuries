...
- **Adaptive granularity** (Treasuries non-bills linear scale):
  - Span > 104w: tick every 52w, label as `Xy` (integer years).
  - 26w < span ≤ 104w: tick every 13w (~quarterly), label as `Xm`.
  - 8w < span ≤ 26w: tick every 4w (~monthly), label as `Xm`.
  - Span ≤ 8w: tick every 1w, label as `Xw`.
- **TIPS / spread charts** (time scale): `unit` auto-determined by Chart.js based on visible range; callback formats ticks as `Xw` / `Xm` / `Xy` using the same span thresholds (91d / 365d).
...
Zoom is NOT cleared by: **source toggles** (FedInvest, Market), date range filter changes, table sort, legend series hide/show (those call `rescaleToVisible` directly).
