...
- **Weekend synthetic-tick finding (2026-09-12):** the trailing live-quote tick CNBC appends beyond the normal per-minute bars (§ "TIPS latest"/"Treasuries latest" in `1.0_Operation.md`) has a weekend variant not previously documented here. The weekday form mislabels the tick with a *stale* prior timestamp; observed on a Saturday (2026-09-12), the tick instead carried a *rolled-forward* timestamp — the current wall-clock day and time — despite no trading having occurred. Confirmed via a direct fetch of `US10YTIPS`'s `1D` feed: the last bar was `{close: "2.601%", tradeTime: "20260912085700"}` (Saturday 08:57 ET, `%`-suffixed unlike every real bar's bare numeric close), one day after the real last bar at Friday `17:05:00` (`"2.6010"`, no `%`). Both forms share the same underlying marker (`%`-suffixed `close`) and the same fix: `fetchLive` (`app.js`) now tags any such bar `synthetic: true` at parse time, and the day-change `closeP` walk excludes synthetic points from both the `latestDayET` boundary and the backward scan — otherwise the weekend form makes the walk treat the actual last trading day as "the previous day" and anchor `closeP` to a bar inside that same day's own session (observed: a spurious `+0.003%` day-change for `US10YTIPS` instead of the correct `+0.055%`).
