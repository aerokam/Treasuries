...
of findings; [2.1 Assemble range data](./2.1_Assemble_Range_Data.md) and [2.3 Calculate day change](./2.3_Calculate_Day_Change.md) reference it.

## TL;DR

- CNBC's Treasury/TIPS data source is **Tradeweb**.
- The CNBC `1D`/`5D` chart feeds are **24-hour continuous** (electronic/overnight).
- Each weekday the day session ends with one **fixed-time consolidation bar per symbol, timestamped `17:05` ET**, followed by a ~2.5h gap (resume ~19:30 ET). This is the *actual end-of-session close*.
...
The `17:05` timestamp is **fixed-time, not a random last trade** — it appears for every symbol every weekday regardless of liquidity, even after a `17:04` bar, even when continuous trading stopped minutes earlier.
...
Comparing the daily-feed close (midnight-timestamped value used for history) against intraday values at 15:00 / 16:00 / 16:59 / 17:05, the daily close consistently matches **15:00 ET** within 0–2bp — **not** 17:05.
...
- **Resolved (2026-09-12) — day-change `closeP` anchor:** now references the 17:05 session close, matching the basis 2D/10D charts already use (was `≤17:00`/~16:59). See [2.3 Calculate day change](./2.3_Calculate_Day_Change.md).
- **Weekend synthetic-tick finding (2026-09-12 to 2026-09-13):** the trailing live-quote tick CNBC appends beyond the normal per-minute bars (§ "TIPS latest"/"Treasuries latest" in [2.2 Read live quotes](./2.2_Read_Live_Quotes.md)) has a weekend variant not previously documented here. The weekday form mislabels the tick with a *stale* prior timestamp; observed on a Saturday (2026-09-12), the tick instead carried a *rolled-forward* timestamp — the current wall-clock day and time — despite no trading having occurred (e.g. `US10YTIPS`'s `1D` feed: last bar `{close: "2.601%", tradeTime: "20260912085700"}`, one day after the real last bar at Friday `17:05:00`). Two attempts at detecting this tick by its *shape* (a `%`-suffixed close, then a bar-to-bar gap-isolation check) both proved fragile — CNBC reformats an older tick's close to a bare numeric string indistinguishable from a real bar once a newer tick supersedes it (observed: the Saturday tick had "matured" this way by Sunday 2026-09-13, defeating the `%` check and reproducing the bug a second time). **Resolved by not detecting the tick's shape at all**: there is never a real trading close on a weekend, so `closeP`'s walk (`app.js`) now simply excludes any weekend-dated point (`isWeekendEt`) — the same filter the 2D/10D chart data itself already applies — rather than guessing at CNBC's inconsistent formatting. See [2.3 Calculate day change](./2.3_Calculate_Day_Change.md) for the full account, including the two superseded approaches. Bond-market holidays get the same exclusion via the already-fetched `saHolidaySet` (`isHolidayEt`, wired up 2026-09-13).
