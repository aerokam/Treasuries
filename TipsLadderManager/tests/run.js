...
// ── Test: Multi-account — no 2040 buys in Acct26 (taxable joint) (taxable) ─────────────
// Load FidelityAllAccounts.csv, select Acct24 IRA + Acct26, rebalance.
// 2040 is long-tier → must go to tIRA (Acct24 IRA), never taxable (Acct26 (taxable joint)).
{
  const fidelityPath = path.resolve('tests/FidelityAllAccounts.csv');
  {
    console.log('\nMulti-account — Acct24 IRA + Acct26: no 2040 buys in taxable');

    const OWNER4_IRA   = 'Acct24 IRA';
    const JOINT       = 'Acct26';
    ...
    if (!ok) {
      ...
      console.error(`\n  Acct24 IRA budget:          $${(iraFlows.budget || 0).toLocaleString('en-US', { maximumFractionDigits: 0 })}`);
      console.error(`  Acct24 IRA allocation cost: $${(iraFlows.newAllocationCost || 0).toLocaleString('en-US', { maximumFractionDigits: 0 })}`);
      const headroom = (iraFlows.budget || 0) - (iraFlows.newAllocationCost || 0);
      console.error(`  Acct24 IRA headroom:        $${headroom.toLocaleString('en-US', { maximumFractionDigits: 0 })}`);

      // Acct24 IRA 2040 position
      for (const cusip in (alloc[OWNER4_IRA] || {})) {
        ...
        if (qtyAfterIRA > 0 || qtyBeforeIRA > 0) {
          console.error(`  Acct24 IRA ${TARGET_YEAR}: ${cusip}  before=${qtyBeforeIRA}  after=${qtyAfterIRA}  delta=${qtyAfterIRA - qtyBeforeIRA}`);
        }
      }
      ...
      // Acct24 IRA sells (what freed budget)
      console.error(`\n  Acct24 IRA sells (budget freed):`);
      let totalFreed = 0;
      for (const cusip in (alloc[OWNER4_IRA] || {})) {
      ...
      console.error(`  Acct24 IRA headroom after allocation: $${headroom.toLocaleString('en-US', { maximumFractionDigits: 0 })}`);
