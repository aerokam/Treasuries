...
**Why yield is computed from price rather than read from the broker's own quoted yield column:** verified empirically (2026-07) that `yieldFromPrice()` produces a more accurate yield than the quoted ask-yield-to-maturity field for TIPS. Matches the convention YieldCurves already uses for TIPS (`buildTipsSecurities` in `YieldCurves/src/app.js`).
