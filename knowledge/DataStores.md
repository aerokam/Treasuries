...
**Logic**: Loads the same R2 inputs the YieldCurves app loads (S1, S4, S7, `misc/BondHolidaysSifma.csv`) and runs every step through the module `YieldCurves/src/app.js` imports for it, so the app and this pipeline cannot drift apart: `shared/src/fidelity-parse.js#parseFidelityNominalRows` for the S7 nominal Treasury rows, `shared/src/tips-pricing.js#priceTips` for the priced TIPS, `shared/src/spot-curve.js#spotCurveFit` for the fits, `shared/src/breakeven.js#findClosestNominal` for the nominal each [S14](#s14) row is stated against, and `shared/src/bond-math.js#termYears` for `Term (y)`. The script evaluates the fit objects' own `z(t)`/`sane()` on the term grid rather than refitting.
