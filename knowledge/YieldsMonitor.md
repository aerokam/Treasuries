...
Fetches all data from CNBC API using the correct `timeRange` parameters per API Mapping spec.
- **Goal**: Ensure consistent data delivery across all time ranges and always current to latest CNBC feed.
- **Method**: Fetch with mapped provider ranges (e.g., 1Y UI range → 1M provider range). All data sources are remote only; no local fallbacks.
...
- **[Operation Manual (1.0)](../YieldsMonitor/knowledge/1.0_Operation.md)**: Exhaustive details on timezone handling, shading colors, and API integration.
- **[API Mapping](../YieldsMonitor/knowledge/API_Mapping.md)**: Required mapping of UI time ranges to CNBC `timeRange` parameters. App fetches from CNBC only.
- **[Data Pipeline](../../knowledge/Data_Pipeline.md)**: Details on the **Yield History Snap** automation script (not the app)—snapshots daily closes for reference/analysis only.
