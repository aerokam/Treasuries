...
**P** &nbsp; [P+I](#pi) &middot; [P+I per TIPS](#pi-per-tips) &middot; [Par Value (Adjusted)](#par-value-adjusted) &middot; [Par Value (Nominal)](#par-value) &middot; [Par Yield](#par-yield) &middot; [PIMCO fund-detail API (E9)](#e9) &middot; [pre ladder interest](#pre-ladder-credit) *(see Pre-Ladder Interest (PLI))* &middot; [Pre-Ladder Interest (PLI)](#pre-ladder-credit) &middot; [Price](#price)
...
**V** &nbsp; [Vanguard Advisors API (E7)](#e7) &middot; [View Selections](#view-selections)
...
- <a id="e3"></a>**FiscalData API (E3)** = `CUSIP + Auction_Date + Security_Type + High_Yield + Bid_to_Cover + ...`
  *U.S. Treasury official auction results and immutable security metadata (Coupons, Dated Dates).*
- <a id="e4"></a>**BLS Public API (E4)** = `Year + Month + Value + Seasonal_Adjustment_Flag`
  *Consumer Price Index (CPI-U) monthly data. Used to derive SA factors by comparing NSA vs. SA values.*
- <a id="e5"></a>**CNBC GraphQL (E5)** = `Symbol + TimeRange + { TradeTime + Close }`
  *The chart-bar feed behind CNBC's own Treasury/TIPS charts (`webql-redesign.cnbcfm.com/graphql`, operation `getQuoteChartData`), sourced from Tradeweb. One bar per `TradeTime` at the resolution `TimeRange` selects (`1D` down to one minute, `5D` to five, coarser tiers to a day or a quarter), `Close` the yield at that bar. Symbols include `US10Y`, `US30YTIPS`, and every other maturity [Yields Monitor](../YieldsMonitor/knowledge/2.1_Assemble_Range_Data.md) tracks. Compared against broker quotes on the same securities, the values sit closer to the bid than to the ask wherever the two differ — an observation from that comparison, not a documented property of the feed (see [Close Price Investigation §4](../YieldsMonitor/knowledge/Close_Price_Investigation.md#4-provenance)), so this is not a mid-price feed as an earlier version of this entry stated.*
- <a id="e15"></a>**CNBC Quote Service (E15)** = `Symbol + Last + LastTime + PreviousDayClosing + MaturityDate + Coupon`
  *CNBC's batched REST quote endpoint (`quote.cnbc.com/quote-html-webservice/restQuote/symbolType/symbol`, one request for every symbol via a `|`-separated list), a different service from CNBC GraphQL (E5) and not itself a chart-bar feed. `Last`/`LastTime` are the live yield and the time it was read, refreshed independently of E5's chart bars and current even when E5's feed is frozen. `PreviousDayClosing` is an official settlement-quote reference distinct from either E5's daily close or [Yields Monitor](../YieldsMonitor/knowledge/2.3_Calculate_Day_Change.md)'s own 17:05 ET session-close reference. `MaturityDate`/`Coupon` identify the one real TIPS a canonical-maturity symbol (e.g. `US2YTIPS`) is quoting today.*
...
- <a id="e7"></a>**Vanguard Advisors API (E7)** = `CUSIP + Holding_Name + Ticker + Category + ( Quantity | Face_Amount ) + Coupon_Rate + Percent_Of_Fund + Market_Value + Maturity_Date + ISIN + SEDOL + As_Of_Date`
...
- <a id="s18"></a>**Intraday archive (S18)**, `intraday-raw/` = `{ @Symbol + @Date + { Feed: { TimeRange + { TradeTime + Close } } } + FetchedAtET }`
  *One immutable daily snapshot per symbol of the raw feeds [CNBC GraphQL (E5)](#e5) served that day, plus the wall-clock time the snapshot was actually taken (`FetchedAtET`) — the last-resort source when E5 itself returns nothing for a symbol. R2 key prefix: `Treasuries/yields-history/intraday-raw/`.*
...
- **Calculated (NSA fallback + educational):** 31 CFR App. B interpolation of the monthly CPI-U NSA series ([BLS Public API (E4)](#e4)). For **NSA only**, this is a **fallback** used if retrieval is unavailable, and is retained for educational value. The retrieved and calculated NSA series **must agree** (verified by test).
...
- <a id="monthly-cpi-u"></a>**Monthly CPI-U** = [BLS Public API (E4)](#e4)
- <a id="market-yields"></a>**Market Yields** = [CNBC GraphQL (E5)](#e5)
- <a id="market-quotes"></a>**Market Quotes** = [Fidelity Fixed Income (E6)](#e6)
- <a id="live-quotes"></a>**Live Quotes** = [CNBC Quote Service (E15)](#e15)
- <a id="fund-holdings"></a>**Fund Holdings** = `[ Vanguard Advisors API (E7) | fminvest.com API (E8) | PIMCO fund-detail API (E9) | Schwab Asset Management holdings export (E10) | BondBloxx product-page holdings table (E11) | BlackRock iShares fund-document API (E12) ]`
...
- <a id="source-data"></a>**Source Data** = `FedInvest_Daily_Price_List + Daily_Ref_CPI + Auction_Results + TIPS_Reference_Data + Monthly_CPI_U + Market_Yields + Market_Quotes + Fund_Holdings + GSW_Curve_Parameters + Tentative_Auction_Schedule`
  *Every flow from an external entity that process 1 reads, drawn as one flow into it at Level 1. [Live Quotes](#live-quotes) is not part of it: no acquisition job reads [CNBC Quote Service (E15)](#e15) — [Yields Monitor](../YieldsMonitor/knowledge/2.2_Read_Live_Quotes.md) reads it directly, the one flow at Level 1 that enters an app rather than process 1.*
