...
- <a id="s17"></a>**Monthly CPI (S17)**, `CPI.csv` = `{ @Year + @Period + PeriodName + NSA + SA }` *— the [Monthly CPI-U](#monthly-cpi-u) flow at rest, 2019 to present: same fields as [CPI history (S8)](#s8) but a narrower window, sufficient to bracket the App. B daily interpolation that produces [Ref CPI NSA and SA (S4)](#s4). R2 key: `bls/CPI.csv`.*
