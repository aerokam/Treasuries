...
- <a id="e5"></a>**CNBC GraphQL (E5)** = `Symbol + Timestamp + Price + Change + Yield`
...
- <a id="tips-yields"></a>**TIPS Yields** = `{ [ TIPS_Prices | TIPS_Quotes ] + Ask_Yield + ( Bid_Yield ) + Index_Ratio }`
  *Each TIPS with the [yield](#yield) calculated from its price at its settlement date, and its [Index Ratio](#index-ratio) calculated at that same date ([3.1.7](../YieldCurves/knowledge/3.1_Parse_Sources_And_Calculate_Yields.md#calculate-tips-yields)) — not the one a market quote states; [Market quotes (S7)](#s7)'s own `Inflation factor` column is parsed but not used. A quoted TIPS also has the yield of its bid price. From 3.1.7 to 3.2, 3.6 and 3.7.*
