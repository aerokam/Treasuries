...
2. **88 stale spec-to-code references** across 61 specs, from `node scripts/check-spec-code.cjs`. Not wired into the pre-commit hook, because 88 findings would block every commit before triage. Concentrations: `3.2_Multi_Account_Rebalancing.md` 27, `3.0_TIPS_Ladder_Rebalancing.md` 17, `AMD_FORMULA_ANALYSIS.md` 14, `2.0_TIPS_Ladders.md` 8. Verified genuine: specs say `activeLowerWeight` where the code has `activeFloorWeight`, `MAX_LAST_YEAR` where it has `maxLastYear`, `fyQtyBefore` where it has `fundedYearQtyBefore` — the last being the `fy` abbreviation the root `CLAUDE.md` retired.
