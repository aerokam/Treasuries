...
9. **Rename 1.1 and 1.1.2 to the whole of what each does** (developer, 2026-09-14): a process name covers its full scope, not its first step. 1.1 becomes Download FedInvest prices and calculate yields. 1.1.2 selects the securities, chooses the price and adds the TIPS reference data of S2, so Select TIPS and Treasury prices falls short; proposed: Select prices and add TIPS reference data. Rename the spec file and its title, every link and `// spec:` tag naming it, and the §2.0 naming rule wording, then regenerate the diagrams, in one commit.
