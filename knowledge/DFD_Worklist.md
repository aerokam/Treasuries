...
6. **`Horizon_Confidence_Weight` is a new term awaiting approval.** Defined in `b20dafc` so that `1.0` and `1.1 §5` would not name an undefined quantity, but the developer has not approved the name. A rename moves the Data Dictionary entry, both specs and `shared/src/ref-cpi.js#seasonalHorizonWeight` together. Two established alternatives, either of which avoids coining a term: **shrinkage weight**, the general statistical name for a factor pulling an estimate toward a prior; and **credibility factor**, the actuarial name, which is the closer match — Bühlmann credibility is `Z = τ² / (τ² + σ²/n)`, the same form as `A² / (A² + σ_drift²)` at `n = 1`, with amplitude for the between-risk variance and drift for the within-risk variance.
7. **`updateSaSaoYields.js` publishes corrected SAO on its next scheduled run.** The duplicate algorithm was retired in `f57ef4a`; measured against the published file, SA moved on none of 53 securities and SAO on 21, by up to 56 basis points, concentrated at the short end where the flat-hold fix applies.
