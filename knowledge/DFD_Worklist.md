...
- **Lead with what the process does.** Not with what a file is or is not. Name a store by its title with its identifier in parentheses, FedInvest prices (S1), never "the file" and never the bare identifier alone. This applies everywhere a store is named in prose, not only at first mention in a document.
...
10. **Store references everywhere use the full name with the identifier in parentheses**, per the revised §2.0 rule: FedInvest prices (S1), not a bare S1. `knowledge/DATA_DICTIONARY.md` and `knowledge/DataStores.md` are done. Still to sweep: the seven `YieldCurves/knowledge/3.*.md` specs, `knowledge/1.1_Download_FedInvest_Prices.md`, the generated-text strings in `scripts/build-dfd.cjs` (captions, aria-labels — regenerate the HTML after), and every other app's `knowledge/*.md`. A bare identifier inside a link label already pointing at the store's own anchor from directly beneath that store's own heading (the "Composition: [Data Dictionary S1]" cross-references in DataStores.md) is the one case left as-is — it is not a fresh naming, it is a same-entry self-reference.
