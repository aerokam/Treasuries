...
20. **Logged, not acted on.** `3.7 Render charts and tables`' framing understates that part of its work is already shared: `shared/src/chart-keys.js` (keyboard pan/zoom for Chart.js) and `shared/src/chart-time-axis.js` (time-axis unit selection) are imported by `CpiExplorer`, `YieldCurves` and `YieldsMonitor` alike. The bulk of the work — building the datasets, the DOM, the table HTML — is still separate code in each app's own `app.js`, not a shared module, so 3.7 isn't wrong to be a YieldCurves-numbered process, only incomplete in not naming the shared pieces it already draws on. Surfaced while starting to propagate the new spec format to 3.2-3.7 (2026-09-18).
