...
10. **Store and entity references everywhere use the full name with the identifier in parentheses**, per the revised §2.0 rule: FedInvest prices (S1) and FedInvest (E1), not a bare S1 or E1. Done: `knowledge/DATA_DICTIONARY.md`, `knowledge/DataStores.md`, the seven `YieldCurves/knowledge/3.*.md` specs, `knowledge/1.1_Download_FedInvest_Prices.md`, and the generated-text strings in `scripts/build-dfd.cjs` (captions, aria-labels; HTML regenerated). Still to sweep: every other app's `knowledge/*.md`. Two cases are left as-is: a bare identifier inside a link label already pointing at the store's own anchor from directly beneath that store's own heading (the "Composition: [Data Dictionary S1]" cross-reference in DataStores.md), and a range naming several same-kind entities at once ("same as E7-E9"), both same-entry or already-named cases rather than a fresh bare naming.
